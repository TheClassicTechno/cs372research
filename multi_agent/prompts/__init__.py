...
def build_round_robin_prompt(
    role: str,
    context: str,
    current_proposal: str,
    section_order: list[str] | None = None,
    prompt_file_overrides: dict[str, str] | None = None,
    allocation_mode: bool = True,
    user_sections: list[str] | None = None,
    sector_constraints: str = "",
) -> str:
    """Build the round robin user prompt for an agent to critique and revise."""
    causal = CAUSAL_CLAIM_FORMAT
    uncertainty = FORCED_UNCERTAINTY

    overrides = prompt_file_overrides or {}
    template_name = overrides.get("round_robin_template", "phases/round_robin_critique_revise.txt")
    ...
    if "_unsectioned" in sections:
        _warn_empty_template_vars("_unsectioned", (_TEMPLATE_DIR / template_name).read_text(), template_vars, "round_robin")
        tmpl = _env.get_template(template_name)
        return tmpl.render(**template_vars)

    return _assemble_user_prompt(sections, order, template_vars, overrides, phase="round_robin")
