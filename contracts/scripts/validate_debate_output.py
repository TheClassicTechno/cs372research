...
SCHEMA_PATH = Path("contracts/schemas/debate_output.schema.json")
...
def runtime_validate(debate: Dict[str, Any], metrics: RunMetrics):
    ...
    for ri, rnd in enumerate(rounds):
        ...
        seen_turn_indices_in_round: Set[int] = set()

        for ti, turn in enumerate(turns):
            metrics.turns_processed += 1
            path = f"rounds[{ri}].turns[{ti}]"

            # turn_id (globally unique)
            ...
            # turn_index_in_round (unique within round, non-negative)
            turn_index = turn.get("turn_index_in_round")
            if not isinstance(turn_index, int):
                add_error(metrics, E_DUPLICATE_TURN_INDEX, "turn_index_in_round must be integer", f"{path}.turn_index_in_round")
            else:
                if turn_index < 0:
                    add_error(metrics, E_NEGATIVE_TURN_INDEX, "turn_index_in_round must be >= 0", f"{path}.turn_index_in_round")
                if turn_index in seen_turn_indices_in_round:
                    add_error(metrics, E_DUPLICATE_TURN_INDEX, "Duplicate turn_index_in_round within round", f"{path}.turn_index_in_round")
                seen_turn_indices_in_round.add(turn_index)
            ...
            for j, attempt in enumerate(attempts):
                ...
                # recommendation_hint sanity (lives on attempt, not turn)
                rec = attempt.get("recommendation_hint")
                if isinstance(rec, dict):
                    ...
                    if isinstance(ps_min, (int, float)) and isinstance(ps_max, (int, float)) and ps_min > ps_max:
                        add_error(metrics, E_RECOMMENDATION_RANGE_INVALID, "position_size_pct_min > position_size_pct_max", f"{a_path}.recommendation_hint")
