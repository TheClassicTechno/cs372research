#!/usr/bin/env python3
"""Run all combinations of debate configs x scenarios for ablation 11.

Ablation 11: 3-agent debates (macro, technical, value_enriched_intense)
with JS collapse threshold 0.6.  Tests whether replacing the risk agent
(ablation 8) with a value agent and lowering the threshold from 0.8 to
0.6 recovers the financial signal seen in the 2-agent ablation 7.

Scans:
    vskarich_ablations/ablation_11/debate_configs/*.yaml

Scenarios loaded from:
    config/scenarios/top_divergence/top_scenarios_l.json  (35 scenarios)

For each (debate, scenario) pair, runs:
    python run_simulation.py --agents <debate> --scenario <scenario>

Tracks progress in ablation_status.json so runs can be resumed after
interruption. Only pairs not already marked "done" are executed.

Usage:
    python vskarich_ablations/ablation_11/run_ablation.py                    # resume / run remaining
    python vskarich_ablations/ablation_11/run_ablation.py --reset            # wipe status, start fresh
    python vskarich_ablations/ablation_11/run_ablation.py --workers 4        # parallel execution
    python vskarich_ablations/ablation_11/run_ablation.py --retries 2        # retry failed runs up to 2x
"""
...
def _load_scenarios() -> list[Path]:
    """Load scenario paths from the top_scenarios_l.json file."""
    data = json.loads(SCENARIO_LIST.read_text(encoding="utf-8"))
    return [REPO_ROOT / p for p in data["scenarios"]]
...
def _run_key(debate_stem: str, scenario_path: Path) -> str:
    return f"{debate_stem}  x  {scenario_path.stem}"
...
# ---------------------------------------------------------------------------
# Single-run execution (used by both serial and parallel paths)
# ---------------------------------------------------------------------------
    ...
    for attempt in range(1, attempts + 1):
        try:
            ...
            if result.returncode == 0:
                return {
                    "key": _run_key(debate_name, Path(scenario_path)),
                    "ok": True,
                    "exit_code": 0,
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Run ablation 11 sweep.")
    parser.add_argument(
    ...
    scenario_configs = _load_scenarios()

    if not debate_configs:
        print(f"No debate configs found in {DEBATE_DIR}")
        sys.exit(1)
    if not scenario_configs:
        print(f"No scenarios found in {SCENARIO_LIST}")
        sys.exit(1)

    # Interleave: group by scenario so baseline and intervention for each
    # scenario run back-to-back, giving paired results early.
    pairs = [(d, s) for s in scenario_configs for d in debate_configs]
    total = len(pairs)
    ...
    remaining = [
        ...
        if status.get(_run_key(d.stem, s), {}).get("result") != "done"
    ]
    already_done = total - len(remaining)

    print(f"Ablation 11 sweep: {len(debate_configs)} debate configs x "
          f"{len(scenario_configs)} scenarios = {total} total")
    if already_done:
    ...
    if args.workers <= 1:
        pbar = tqdm(total=len(remaining), desc="Ablation 11", unit="run")
        ...
        with ThreadPoolExecutor(max_workers=args.workers) as executor:
            ...
            for debate, scenario in remaining:
                ...
                future_to_key[future] = _run_key(debate.stem, scenario)
