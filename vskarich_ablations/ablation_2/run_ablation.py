#!/usr/bin/env python3
"""Run all combinations of debate configs × scenario configs for ablation 2.

Scans:
    vskarich_ablations/ablation_2/debate_configs/*.yaml
    vskarich_ablations/ablation_2/scenario_configs/*.yaml
...
Usage:
    python vskarich_ablations/ablation_2/run_ablation.py                    # resume / run remaining
    python vskarich_ablations/ablation_2/run_ablation.py --reset            # wipe status, start fresh
    python vskarich_ablations/ablation_2/run_ablation.py --workers 4        # parallel execution
    python vskarich_ablations/ablation_2/run_ablation.py --retries 2        # retry failed runs up to 2x
"""
...
def _execute_run(
    ...
    Returns a result dict (does NOT write to status file — that's the
    caller's responsibility to avoid race conditions).
    """
    ...
    last_exit = -1
    attempts = retries + 1  # total attempts = retries + 1
    elapsed = 0
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Run ablation 2 sweep.")
    parser.add_argument(
    ...
    # Determine which pairs still need to run
    remaining = [
    ...
    print(f"Ablation 2 sweep: {len(debate_configs)} debate configs x "
          f"{len(scenario_configs)} scenario configs = {total} total")
    if already_done:
    ...
    if args.workers <= 1:
        # ---- Serial execution ----
        pbar = tqdm(total=len(remaining), desc="Ablation 2", unit="run")
        ...
        with ProcessPoolExecutor(max_workers=args.workers) as executor:
            ...
            for future in as_completed(future_to_key):
                ...
                # Update status (main thread only — no race conditions)
                status[run_result["key"]] = {
                    "result": "done" if run_result["ok"] else "failed",
                    "exit_code": run_result["exit_code"],
                    "elapsed_s": run_result["elapsed_s"],
