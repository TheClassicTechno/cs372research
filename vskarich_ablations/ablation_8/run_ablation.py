#!/usr/bin/env python3
"""Run all combinations of debate configs x scenarios for ablation 8.

Ablation 8: 3-agent debates (macro, technical, risk_enriched_intense)
with the same 35 scenarios and 2 debate configs (baseline vs strict
intervention) as ablation 7.

Scans:
    vskarich_ablations/ablation_8/debate_configs/*.yaml
...
Usage:
    python vskarich_ablations/ablation_8/run_ablation.py                    # resume / run remaining
    python vskarich_ablations/ablation_8/run_ablation.py --reset            # wipe status, start fresh
    python vskarich_ablations/ablation_8/run_ablation.py --workers 4        # parallel execution
    python vskarich_ablations/ablation_8/run_ablation.py --retries 2        # retry failed runs up to 2x
"""
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Run ablation 8 sweep.")
    parser.add_argument(
    ...
    print(f"Ablation 8 sweep: {len(debate_configs)} debate configs x "
    ...
    if args.workers <= 1:
        pbar = tqdm(total=len(remaining), desc="Ablation 8", unit="run")
