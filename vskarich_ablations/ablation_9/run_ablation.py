#!/usr/bin/env python3
"""Run all combinations of debate configs x scenarios for ablation 9.

Ablation 9: 2×2 factorial design (PID × JS Intervention), 2 agents,
2 rounds, 35 scenarios = 140 total runs.

  Configs:
    nopid_nointervention   — control (no PID, no intervention)
    nopid_intervention     — JS intervention only (threshold 0.8)
    pid_nointervention     — PID only
    pid_intervention       — PID + JS intervention

Scans:
    vskarich_ablations/ablation_9/debate_configs/*.yaml
...
Usage:
    python vskarich_ablations/ablation_9/run_ablation.py                    # resume / run remaining
    python vskarich_ablations/ablation_9/run_ablation.py --reset            # wipe status, start fresh
    python vskarich_ablations/ablation_9/run_ablation.py --workers 4        # parallel execution
    python vskarich_ablations/ablation_9/run_ablation.py --retries 2        # retry failed runs up to 2x
"""
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Run ablation 9 sweep.")
    parser.add_argument(
    ...
    print(f"Ablation 9 sweep: {len(debate_configs)} debate configs x "
    ...
    if args.workers <= 1:
        pbar = tqdm(total=len(remaining), desc="Ablation 9", unit="run")
