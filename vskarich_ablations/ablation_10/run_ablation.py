#!/usr/bin/env python3
"""Run all combinations of debate configs x scenarios for ablation 10.

Ablation 10 — CRIT Causal Pillar Intervention
==============================================
Tests whether forcing agents to retry their revision when CRIT detects
causal_alignment < 0.75 improves causal reasoning quality.

Design:
    2 debate configs (baseline vs causal_intervention_th075)
    × 35 scenarios
    = 70 total runs

    Baseline: CRIT runs but reasoning_quality rule disabled (never fires)
    Treatment: reasoning_quality enabled with causal_alignment threshold 0.75
               for both macro and technical agents

Scans:
    vskarich_ablations/ablation_10/debate_configs/*.yaml
...
Usage:
    python vskarich_ablations/ablation_10/run_ablation.py
    python vskarich_ablations/ablation_10/run_ablation.py --reset
    python vskarich_ablations/ablation_10/run_ablation.py --workers 4
    python vskarich_ablations/ablation_10/run_ablation.py --retries 2
"""
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Run ablation 10 sweep.")
    parser.add_argument(
    ...
    debate_configs = sorted(DEBATE_DIR.glob("*.yaml"), reverse=True)
    ...
    print(f"Ablation 10 — CRIT Causal Pillar Intervention")
    print(f"  {len(debate_configs)} debate configs x "
    ...
    if args.workers <= 1:
        pbar = tqdm(total=len(remaining), desc="Ablation 10", unit="run")
