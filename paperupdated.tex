...
Quant trading firms such as Jane Street traditionally leverage statistical, quantitative models that forecast stock price movements. But this approach is constrained by its inability to capture the full complexity of market factors, from qualitative sources such as earnings reports and financial news. Recent advances in Large Language Models (LLMs) have motivated LLM-based trading products that can synthesize information and produce transparent decision-making on such textual sources \citep{2025tradingagents}. However, despite their promise, LLMs remain susceptible to reasoning failures such as sycophancy, overconfidence, hallucinated causality, and groupthink, which can undermine the reliability and soundness of investment decisions. 
...
Our motivation is to explore whether structured disagreement and causal control of debate can improve investment performance by enhancing reasoning quality. Markets are complex environments with a high number of confounding factors. For LLM agents managing investment portfolios, we hypothesize that: 
\begin{enumerate}[label=H\arabic*.]
    \item  \label{H1} \textbf{Measurement Validity:} Reasoning-evaluative metrics are economically predictive. 
    \item  \label{H2} \textbf{Controllability:} Reasoning quality can be improved by debate and  control signals.
    \item  \label{H3} \textbf{Causal Regulation:} Controlling causal reasoning depth (i.e., using the correct  Pearl causality rung \citep{pearl2009causality} significantly improves portfolio performance.


\end{enumerate}


We will compare various LLM agent configurations within a stock-market simulation using real-world market narratives and price data, with and without blind intervention. Through analysis of reasoning traces, we will evaluate whether improved reasoning quality (via T³/CRIT-style rubrics \citep{chang2023crit, raudit}) correlates with better risk-adjusted returns. If debate improves performance, it supports the mechanism for decision-making under uncertainty; if not, we can distill the most effective constraints into stronger single-agent policies \citep{quantagents2025, tradingagents2024}).
% ------\\
...
We implemented a multi-agent debate system in Python using LangGraph \citep{langgraph}, composed of eight modules: (1) Pydantic data models, (2) configuration layer, (3) role-specific prompt templates, (4) debate graph, (5) runner, (6) demo harness, (7) test suite, and (8) evaluation utilities.
...
\begin{center}
\texttt{START} $\to$ \texttt{news\_digest} / \texttt{price\_history} $\to$ \texttt{build\_context} $\to$ \texttt{propose} $\to$ \texttt{critique} $\rightleftharpoons$ \texttt{revise} $\to$ \texttt{judge} $\to$ \texttt{build\_trace} $\to$ \texttt{END}
\end{center}

Input nodes (\texttt{news\_digest}, \texttt{price\_history}) preprocess raw inputs in parallel. In the loop, agents independently propose trades with causal claims, critique other agents' proposals (including self-critique), and revise. A conditional edge loops critique$\to$revise for a configurable number of rounds before a judge synthesizes a final decision. The judge preserves the strongest dissenting objection even if overruled. The \texttt{build\_trace} node packages the full debate into a JSON trace.

\textbf{Experimental Knobs.}
A \texttt{DebateConfig} dataclass centralizes all parameters: agent roster (any subset of six roles), max debate rounds (default~1), agreeableness $\in [0,1]$ (default~0.3, controlling how confrontational critiques are), adversarial mode (adding Devil's Advocate role), pipeline components, and LLM model configurations (\texttt{gpt-4o-mini}, temperature~0.3). OpenAI API calls are configured with retry logic (3 attempts, exponential backoff). The agreeableness parameter adjusts the tone of the prompts across a spectrum of confrontational to collaborative discussion during debate, which drives down overconfidence and converges towards consensus seeking.
...
Our implementation draws on the RAudit protocol \citep{raudit} for blind process auditing of LLM reasoning, 
which evaluates trace integrity without access to ground truth. 
However, rather than treating reasoning quality as a single composite signal, 
we decompose it into four CRIT-style pillars $P_1$–$P_4$ 
\citep{chang2023crit} and analyze each dimension independently.

We formalize reasoning quality through this four-pillar decomposition. 
In the current milestone, we implement and validate $P_1$ 
(trace-output consistency) as the foundational measurement layer. 
The remaining pillars ($P_2$–$P_4$), together with closed-loop control mechanisms, 
constitute the next stage of development and are described in Future Work.
% , totaling approximately 710 lines,



\textbf{Trace-Output Consistency ($P_1$).}
The \texttt{ConsistencyJudge} evaluates whether each agent's output at every debate turn is logically consistent with its reasoning trace. The judge operates on four turn types -- proposal, critique, revision, and judge decision -- each with a specialized prompt template. For proposals, it checks that the hypothesis, justification, and orders are coherent and mutually supportive. Critiques are assessed for whether objections are grounded in the actual proposal text, and whether the critique addresses the correct logical points. Revisions are checked for whether the stated changes are actually reflected in the revised proposal. Judge decisions are evaluated for whether the final synthesis accurately reflects the strongest arguments and dissent.
...
Our preliminary experiments focus on validating the system's correctness and establishing the ablation infrastructure for exploring our research hypotheses. We have not yet run simulation-integrated experiments; the results on the configurations below characterize the debate system in isolation.
...
\textbf{Sycophancy-Stubbornness Tradeoff.} Preliminary evaluation shows agents often acknowledge critiques but refuse to revise (stubbornness), while high agreeableness produces performative consensus. Prompt-based controls may be brittle across model versions.

\textbf{Shallow Causal Defaults.} Without active prompt constraints, Risk and Technical agents default to L1 (associational) claims rather than the L2/L3 reasoning the system targets.

\textbf{LLM-as-Judge Circularity.} Using \texttt{gpt-4o-mini} to evaluate \texttt{gpt-4o-mini} debate traces risks shared blind spots that are difficult to detect without human validation.

\textbf{Simulation Gap.} Our environment abstracts away slippage, liquidity, and order-book dynamics; results may not transfer to live markets, and non-stationary regimes may invalidate strategies.

\textbf{Metric and Hypothesis Risk.} If H1 fails (reasoning quality does not predict returns), PID control framework (H2, H3) lacks meaningful signal. Specifically, it is possible that due to the simulation gap, sticking to any `answer key' available in the pretraining data could triumph \emph{without} reason.

\vspace{-6pt}
\section{Future Work}

\textbf{1. Simulation Integration.}
Integrate the debate system with a market simulator so trades execute against real price trajectories, enabling direct PnL, Sharpe, and drawdown measurement.
...
Explore mechanisms to encode regret on past trade decisions to enhance future reasoning.
...
% 	\textbf{1. Simulation Integration.}
% Integrate the multi-agent debate system with a dynamic market simulator, enabling debate-generated trades to execute against realistic price trajectories. This will allow direct measurement of PnL, Sharpe ratio, drawdown, and other financial metrics alongside reasoning quality.

% 	\textbf{2. Automated Quantitative Reasoning Evaluation.}
% In addition to our existing $P_1$ measurement, we will implement evaluators for the remaining CRIT pillars.  These scores will then not only be evaluated but also used in addition to divergence metrics to control the debate with RAudit-style PID.  To extend the work of RAudit, we will separately measure the impact of each CRIT pillar to see which correlates most strongly with the economic measures such as PnL, Sharpe ratio, Sortino ratio, and maximum drawdown.
% % T\textsuperscript{3}/CRIT-style rubric (\cite{chang2023crit,raudit}) to score causal claims, leveraging the Pearl level tags already present in each claim. The evaluator will check for internal consistency of variables, assumptions, and falsifiers, and verify that L2/L3 claims demonstrate genuine interventional or counterfactual reasoning.
...
% \end{equation}
% where $R_i$ denotes realized return and $CI_i$ denotes the Causal Instability Index. We report correlations with . If higher reasoning quality predicts improved risk-adjusted returns or reduced downside risk, then RAudit captures economically meaningful structure rather than superficial coherence. This hypothesis establishes the validity of reasoning measurement.
...
% We test whether PID reduces reasoning instability and increases $\rho$, and whether such improvements coincide with improved risk-adjusted performance. If so, reasoning quality is not merely measurable but actively controllable. This hypothesis establishes reasoning controllability.
...
% RI = robustness index measuring stability under perturbation. If selective $P_4$ regulation increases $\bar{P}_4$, reduces CR and CI, and improves SR or reduces DD under stress without degrading overall coherence ($\rho$), then causal depth is both controllable and economically meaningful.
% \subsection{Summary and Other Future Work}

% Together, these hypotheses test a progression: 1. Whether reasoning quality predicts economic outcomes, 2. Whether reasoning quality can be improved through control, 3. Whether causal reasoning depth can be selectively regulated to improve robustness. The evaluation therefore moves from measurement to control to structured causal regulation, with financial performance serving as the ultimate validation signal.

% 	\textbf{3. Correlation \& Failure-Mode Analysis.}
% Test whether reasoning quality metrics (consistency, causal depth) correlate with risk-adjusted returns (RQ2). Analyze outputs across agreeableness and adversarial setups to measure reductions in overconfidence, sycophancy, groupthink (RQ3).
...
% Expand experiments to a broader stock universe, longer time horizons, and alternative LLM backends. Assess scalability, cost, and performance tradeoffs as system is scaled up. 
% Apply the existing consistency checker and divergence analyzer across all eight ablation configurations (Table~\ref{tab:ablation-configs}) to quantify reductions in sycophancy, stubbornness, and groupthink as debate parameters vary.
...
% 	\textbf{8. Full Ablation Comparison.}
% Compare four architecture conditions on identical market scenarios: (i) single-agent, (ii) majority-vote, (iii) basic multi-agent debate, and (iv) controlled debate (debate + RAudit PID) to answer whether structured debate-and reasoning control-improve decision quality over simpler aggregation baselines.
